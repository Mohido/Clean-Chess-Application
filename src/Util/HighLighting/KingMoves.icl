...
| xCoordinate <> 5 && ( yCoordinate <> 0 || yCoordinate <> 7)  = pst //if king is not already on it's orignal positon, we just return the current pst
= case ls.worldMatrix.[xCoordinate + yCoordinate * 8] of
	Nothing = case ls.worldMatrix.[(xCoordinate + 1) + yCoordinate * 8] of
				Nothing = case ls.worldMatrix.[(xCoordinate + 2) * 8] of 
							Nothing = pst 
							Just piece = case (piece.player == clickedPiece.player) of
											False = pst 
...
| xCoordinate <> 3 && ( yCoordinate <> 0 || yCoordinate <> 7)  = pst //if king is not already on it's orignal positon, we just return the current pst
= case ls.worldMatrix.[xCoordinate + yCoordinate * 8] of
	Nothing = case ls.worldMatrix.[(xCoordinate-1) + yCoordinate * 8] of
...
>>>>>>> e3170fd0f0306bb436f1d729df0dd204968e8a4a
