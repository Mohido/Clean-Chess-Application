...
where
	...
	newPst     = {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io}
...
= case ls.worldMatrix.[xC + yC * 8] of 
	Nothing = goLeftRook (xC-1) yC p {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io}  
	Just piece = case (piece.player == p.player) of
					False = {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io} // if not the same piece Highlight and stop
					True = pst
where
	point = {x = xC * TILE_SIZE , y = yC * TILE_SIZE}
...
| xC > 7 = pst
= case ls.worldMatrix.[xC + yC * 8] of 
	Nothing = goRightRook (xC+1) yC p {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io}  
	Just piece = case (piece.player == p.player) of 
					False = {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io} // if not the same piece Highlight and stop
					True = pst
where
...
| yC > 7 = pst
= case ls.worldMatrix.[xC + yC * 8] of 
	Nothing = goForwardRook xC (yC+1) p {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io}  
	Just piece = case (piece.player == p.player) of 
					False = {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io} // if not the same piece Highlight and stop
					True = pst
where
...
| yC < 0 = pst
= case ls.worldMatrix.[xC + yC * 8] of 
	Nothing = goBackwardRook xC (yC-1) p {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io}  
	Just piece = case (piece.player == p.player) of 
					False = {pst & io = setWindowLook (ls.windowId) True (False, (\_ _ = hiliteAt point tile)) io} // if not the same piece Highlight and stop
					True = pst
where
